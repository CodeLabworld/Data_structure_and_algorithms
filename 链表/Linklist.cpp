...
/*建立非递减有序链表*/
//***************************************************************************
void CreateOrderList(LinkList &L)
{
    LinkList p, r, q;
    int i;
    int number;
    ...
    int min = data[0];
    q = (Node *)malloc(sizeof(Node)); //生成新节点
    L->next = q;
    q->data = min;
    r = q;
    for (i = 1; i < number; i++)
    {
        p = (Node *)malloc(sizeof(Node)); //生成新节点
        p->data = data[i];
        if (p->data > data[i - 1]) //尾插
        {
            r->next = p; //将表尾终端结点的指针指向新节点
            r = p;       //将当前的新节点定义为表尾终端结点
        }
        else if (p->data <= min) //头插
        {
            L->next = p;
            p->next = q;
            q = p;
